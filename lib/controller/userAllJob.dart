...
import 'package:nregauser/model/viewAllJobUserModel.dart';
...
class UserAllJob extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return UserAllJobClass();
  }
}

class UserAllJobClass extends State<UserAllJob> {
                              ...
                              child: Container(
                                  ...
                                  child: Column(
                                    children: <Widget>[
                                      ...
                                      appliedJobId
                                              .contains(viewAllJobList[i].jobId)
                                          ? Center(
                                              child: Text(
  ...
  Future viewJobs() async {
    ...
    internetConnection.then((onValue) {
      if (onValue == false) {
        ...
        sendPostRequest(
                API().dummyUrl + API().viewAllJobUser, {"userId": userId})
            .then((response) {
          ViewAllJobUserModel V =
              ViewAllJobUserModel.fromJson(json.decode(response.body));
          if (V.code == 200) {
  ...
  applyForJob(jId, jname) {
    ...
    applyForjob.then((response) {
      print("iiiii");
      print(response.body);
      AddNewJobModel N = AddNewJobModel.fromJson(json.decode(response.body));
      if (N.code == 200) {
        Navigator.pushReplacementNamed(context, "/useralljob");
      } else {
        print("Job cannot be added");
      }
    });
  }
}
